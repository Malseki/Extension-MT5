...
input bool   InpAvisoCont  = true;   // avisar continuacion (BOS + H1 alineado)
input bool   InpSonido     = true;   // sonido en el aviso de continuacion
...
int      gNEv = 0;
int      gTrend = 0;
int      gTrendH1 = 0;          // tendencia del H1, para la confluencia
datetime gLastCont = 0;         // ultimo aviso de continuacion emitido
int      gNCont = 0;            // avisos de continuacion en esta sesion
...
//+------------------------------------------------------------------+
int Scan(const ENUM_TIMEFRAMES tf, S2Event &evs[], int &nEv)
  {
   nEv = 0; ArrayResize(evs, MAXEV);
      ...
      if(trend == 0)
        {
         if(!originHighBroken && originHigh > 0 && srcUp > originHigh)
           {
            if(nEv < MAXEV) { evs[nEv].t=r[i].time; evs[nEv].level=originHigh;
                               evs[nEv].levelT=originHighT; evs[nEv].kind=3;
                               evs[nEv].price=srcUp; nEv++; }
           ...
           {
            if(nEv < MAXEV) { evs[nEv].t=r[i].time; evs[nEv].level=originLow;
                               evs[nEv].levelT=originLowT; evs[nEv].kind=4;
                               evs[nEv].price=srcDown; nEv++; }
      ...
      else if(trend == 1)
        {
         if(!keyHighBroken && keyHigh > 0 && srcUp > keyHigh)
           {
            if(nEv < MAXEV) { evs[nEv].t=r[i].time; evs[nEv].level=keyHigh;
                               evs[nEv].levelT=keyHighT; evs[nEv].kind=1;
                               evs[nEv].price=srcUp; nEv++; }
            keyHighBroken = true;
           }
      ...
      else
        {
         if(!keyLowBroken && keyLow > 0 && srcDown < keyLow)
           {
            if(nEv < MAXEV) { evs[nEv].t=r[i].time; evs[nEv].level=keyLow;
                               evs[nEv].levelT=keyLowT; evs[nEv].kind=2;
                               evs[nEv].price=srcDown; nEv++; }
            keyLowBroken = true;
           }
         if(!originHighBroken && originHigh > 0 && srcUp > originHigh)
...
//+------------------------------------------------------------------+
//| AVISO DE CONTINUACION — el hueco que el detector no cubria.        |
//|                                                                   |
//| TRADER-ALERT-001 avisa REVERSION: si el precio baja hasta un       |
//| nivel, avisa COMPRA. En una bajada sostenida nunca da una venta.   |
//| Sus otros dos avisos (temprano, impulso) miden movimiento DENTRO   |
//| de una vela M5, asi que una caida grande pero gradual —25 pips     |
//| repartidos en 20 velas, 1,25 por vela— no dispara ninguno.         |
//| El 2026-08-26 el trader vio bajar el precio sin recibir un solo    |
//| aviso, y tenia razon: no habia nada que mirara direccion.          |
//|                                                                   |
//| Esto lo cubre: avisa cuando un BOS confirma continuacion Y el H1   |
//| apunta al mismo lado.                                              |
//|                                                                   |
//| ADVERTENCIA QUE NO HAY QUE OLVIDAR: esto opera A FAVOR del         |
//| movimiento, y E-MT5-034 midio que hacerlo rinde 2,91 pp PEOR que   |
//| operar en contra (44,87% contra 47,78%, n=1052 vs 2631). Como      |
//| herramienta para el criterio del operador es razonable; como       |
//| senal automatica, la evidencia disponible apunta EN CONTRA.        |
//| Se registra para poder medirlo (E-MT5-037), no porque funcione.    |
//+------------------------------------------------------------------+
void CheckContinuacion()
  {
   if(!InpAvisoCont || gNEv <= 0) return;

   int k = gEvs[gNEv-1].kind;
   if(k > 2) return;                       // solo BOS; el CHoCH es reversion
   datetime et = gEvs[gNEv-1].t;
   if(et == gLastCont) return;             // ya avisado

   // Solo eventos frescos: el BOS tiene que ser de las ultimas 2 velas, o al
   // reiniciar el indicador avisaria de golpe todo el historico.
   int per = PeriodSeconds(_Period);
   if(TimeCurrent() - et > 2 * per) { gLastCont = et; return; }

   int dir = (k == 1) ? +1 : -1;
   if(dir != gTrendH1) return;             // sin confluencia con H1, no avisa

   gLastCont = et; gNCont++;
   string d = (dir > 0) ? "COMPRA / BUY" : "VENTA / SELL";
   string msg = StringFormat("CONTINUACION %s | BOS %s en %s | H1 %s | %s",
                  d, (dir > 0 ? "alcista" : "bajista"),
                  DoubleToString(gEvs[gNEv-1].level, _Digits),
                  (gTrendH1 > 0 ? "ALCISTA" : "BAJISTA"), EnumToString(_Period));
   Print("CONTINUACION >>> ", msg, "  [a favor del movimiento: E-MT5-034 lo midio PEOR]");
   if(InpSonido) PlaySound("news.wav");

   int fh = FileOpen("TRADER-CONTINUACION.csv",
                     FILE_READ | FILE_WRITE | FILE_CSV | FILE_ANSI | FILE_COMMON
                     | FILE_SHARE_READ | FILE_SHARE_WRITE, ',');
   if(fh == INVALID_HANDLE) return;
   if(FileSize(fh) == 0)
      FileWrite(fh, "aviso_t", "symbol", "timeframe", "direccion", "bos_level",
                "trend_h1", "precio", "left_right", "nota");
   FileSeek(fh, 0, SEEK_END);
   MqlTick t; SymbolInfoTick(_Symbol, t);
   FileWrite(fh, TimeToString(TimeCurrent(), TIME_DATE | TIME_SECONDS), _Symbol,
             EnumToString(_Period), (dir > 0 ? "BUY" : "SELL"),
             DoubleToString(gEvs[gNEv-1].level, _Digits),
             (gTrendH1 > 0 ? "ALCISTA" : "BAJISTA"),
             DoubleToString(t.bid, _Digits),
             StringFormat("%d/%d", InpLeftBars, InpRightBars),
             "a favor del movimiento - E-MT5-037 sin medir");
   FileClose(fh);
  }
...
void Render()
  {
   gTrend = Scan(_Period, gEvs, gNEv);

   // Tendencia de H1 para la confluencia. Si ya estamos en H1 no se rescanea.
   if(_Period == PERIOD_H1) gTrendH1 = gTrend;
   else
     {
      S2Event tmp[]; int nTmp = 0;
      gTrendH1 = Scan(PERIOD_H1, tmp, nTmp);
     }
